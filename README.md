...
> ReactJS is a library and JS framework that helps solve the problem of wasted resources by increasing reusability.  Also the scalability, allows users to great large single-page applications with very fast, less resource-intensive rendering abilities.

2. Describe component state.
> Component state is where you store the property values of a given component that details what the component should store and how it functions.  This state is never overwritten, but changed using the proper methods packaged with React to avoid mutations.

3. Describe props.
> Props are properties housed by parent elements / components that child components can inherit to be able to use within their packaged logic.  It's importance is to declutter by using a separation of concerns between the parent and child, so that the props on the parent can be passed to many children.

4. What are side effects, and how do you sync effects in a React component to changes of certain state or props?
> Side effects are any kind of logic independent of the rendering within React.  It gives you the ability to give a component instructions upon first render and then instructions when the component has done it's job, what to do afterwards. You can sync certain state or props of a component via `useEffect()`.
